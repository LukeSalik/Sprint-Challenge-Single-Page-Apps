...
- [ ] Explain benefit(s) using `client-side routing`?
      Client side routing makes for a smoother exerience for the user than server side routing. This is due to no reloading of pages and all the routing is handled internally using JavaScript. However server side routing requests data from the server each time there is a change of pages which can among others things make for a slow and clunky user experience.
  > Answer:
